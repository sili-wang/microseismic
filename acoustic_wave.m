...
T=0.5;        % 
wave(round(T/dt))=0;    % 
plot(wave),title('wavelet');

Vv=load('marmsmooth.mat');Vv=Vv.V1;       % �����ٶ�,m/s
% v0=2000;
% Vv=v0*ones(300,300);
...
%     p(z1,x1)=p(z1,x1)+wave(ttt);
%     p(z2,x2)=p(z2,x2)+wave(ttt);
%     p(z3,x3)=p(z3,x3)+wave(ttt);
%     p(z4,x4)=p(z4,x4)+wave(ttt);
%     p(z5,x5)=p(z5,x5)+wave(ttt);   
