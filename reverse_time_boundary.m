...
wave=fliplr(Seis');
wave=wave';
T=0.5;        % transmit time, s


Vv=load('V1.mat');Vv=Vv.V1;       % velocity map, m/s
[Nz Nx] = size(Vv);
pml=50;
m = Nx + 2*pml;
n = Nz + 2*pml;

dz=5;                              % grid steo size on z, m
dx=5;                              % grid step size on z, m
...
for t=dt:dt:T
    ttt = ttt + 1;
    p(z0,pml+channelnum-29:pml+channelnum+30)=wave(ttt,channelnum-29:channelnum+30);
...
k=4:m-4;
    ...
    M(:,:,channelnum/30) = max(Wavefeild,[],3);
end
